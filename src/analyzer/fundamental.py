...
class FundamentalAnalyzer:
    ...
    def __init__(self):
        ...
        if not self.api_key:
            logger.warning("EDINET_DB_API_KEY が未設定。財務データはスキップされます")
    ...
    def get_financial_data(self, ticker: str) -> Optional[dict]:
        ...
        try:
            ...
            if res_ratios.status_code == 200:
                raw = res_ratios.json().get("data", {})
                # リスト形式で返ってくる場合は最初の要素を使用
            ...
            if res_analysis.status_code == 200:
                raw_a = res_analysis.json().get("data", {})
                # リスト形式で返ってくる場合は最初の要素を使用
                if isinstance(raw_a, list):
                    raw_a = raw_a[0] if raw_a else {}
                credit_score = raw_a.get("credit_score")
                ai_comment = raw_a.get("summary", "")
    ...
    def score_financial_health(self, debt_to_equity, current_ratio, equity_ratio=None) -> tuple:
        score = 5
        notes = []
    ...
    def score_credit(self, credit_score) -> tuple:
        ...
        else:              return 1,  f"🤖 AIスコア低い({score}点)"
